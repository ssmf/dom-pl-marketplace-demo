...
const { getVendorOrders } = useVendorService()

const { data: orders, pending } = await useAsyncData(
  `vendor-orders-detail-${vendorId}`,
...
const backUrl = vendorId ? `/konto/sprzedawca?id=${vendorId}` : '/konto/sprzedawca'
</script>
    ...
    <template v-else>
      ...
      <UCard>
        <template #header>
          <h2 class="font-semibold text-default">
            Sprzedane produkty ({{ order.items.length }})
          </h2>
        </template>
      ...
      <UCard>
        ...
        <dl class="grid grid-cols-1 gap-3 sm:grid-cols-2 text-sm">
          ...
          <div>
            <dt class="text-muted">Kupujący (email)</dt>
            <dd class="font-medium text-default mt-0.5">
              {{ order.email || '—' }}
            </dd>
          </div>
